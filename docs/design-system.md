...
**Dashboard Layout**:
- Sidebar: 280px (teacher nav) or collapsible to 64px (icon mode)
- Main content: `calc(100vw - 280px)`, max-width 1400px
- Card padding: `var(--space-lg)` (24px)
- Between cards: `var(--space-xl)` (32px)
...
- Navigation: `shadow-sm` (subtle separation)
...
- Optional: subtle scale 1 to 1.02
- Cursor: pointer
...
### Lesson Status Change (real-time)

- Pulse effect when a lesson is marked "Delivered": 2-3 pulses, then settle
- Color transition (gray to emerald): 300ms

### Modal Open

- Backdrop: fade in 200ms
- Modal itself: fade + scale (0.95 to 1.0) over 300ms
- Close: reverse, 200ms

### Form Validation

- Field-level: red border + icon appears immediately (0ms delay - UX signal)
- Error message: fade in 150ms
- Success checkmark: brief 400ms bounce animation on submit success
...
- Text: white
- Confirmation UI on click (popover asking "Are you sure?")
...
- Content: time (prominent), student name, location, status badge

**Subscription/Payer Card** (detail pages):
- Similar structure
- Status badge top-right (ACTIVE, CANCELLED, PENDING_REVIEW)
- Balance figure emphasized (large, teal if positive)

### Tables (Ledger, Lesson History)

- Header: gray-100 background (light mode), gray-800 (dark)
- Rows: alternating white/gray-50 for scannability
- Hover row: slight background shift, never bold (too aggressive)
- Borders: 1px solid `var(--color-border)` (minimal)
- Padding per cell: `12px`
- Money amounts: right-aligned, monospace font, color-coded (green/red per sign)

### Forms

- Input padding: `10px 12px`
- Border: 1px solid `var(--color-border)`
- Focus: 2px solid `var(--color-primary)` (blue ring)
- Placeholder: gray-400, italic
- Label: bold, gray-700 (light) / gray-300 (dark)
- Error state: border red, error message in red below

### Modals

- Backdrop: rgba(0,0,0,0.5) (semi-transparent)
- Modal: rounded-lg, shadow-xl, white background (light) / gray-800 (dark)
- Max-width: 480px (standard), 680px (wide)
- Padding: `var(--space-xl)` (32px)
- Close button: top-right, icon + hover state
...
- Parent microsite: full-width at all sizes, single-column (no sidebar)
...
- Cards: gray-800 with subtle borders (gray-700) for definition
...
- Test with [WebAIM contrast checker](https://webaim.org/resources/contrastchecker/) before shipping

**Interactive elements**:
- Keyboard navigation: Tab order logical, visible focus indicator (not removed)
- Focus ring: 2px solid `var(--color-primary)` (teal), 2px offset
- Buttons/links: min 44x44px touch target (mobile)

**Text**:
- Line-height: 1.5 minimum (readability)
- Letter-spacing: avoid letter-spacing tightness
- Font size: 16px minimum for body (never smaller than 14px)
- Lists: use `<ul>`, `<ol>` semantically, not divs

**Images & Icons**:
- All img tags: `alt` attribute (functional description)
- Icons without text: `aria-label` or wrapped in labeled button
- Decorative only: `aria-hidden="true"`

**Forms**:
- Every input: associated `<label>` (not placeholder-only)
- Error messages: linked via `aria-describedby`
- Required fields: marked via `required` attribute + visual indicator (asterisk)

**Semantic HTML**:
- `<button>` for buttons, not divs
- `<nav>`, `<main>`, `<section>` for landmarks
- `<table>` for data tables, not layout
- Heading hierarchy: h1 to h2 to h3 (no skipping)
...
- Teacher's name + school/location context
- Quick action buttons: "Create Lesson", "Message Guardian", "Mark Attendance"
- Settings icon (top-right)

### Lesson Card (Dashboard)

- Left border: 4px, [status color]
- Time (prominent, lg font): "Mon 3:30 PM"
- Student name + location (md font)
- Status badge: small pill, teal background, white text
- Hover: shadow-lg, pointer becomes active
- Click: navigate to lesson detail page

### Ledger Entry (Table row)

- Date (left) | Description (center) | Amount (right)
- Description: "Lesson: Flute" or "Refund (Cancellation)" or "Payment received"
- Amount: monospace, green if credit (+), red if charge (-), right-aligned
- Alternating row backgrounds for scannability

### Parent Microsite (After login)

- Clean, minimal: student's name + schedule week view + balance + next lesson
- Large readable type (Lora body, 18px)
- One CTA per page (e.g., "View full schedule", "Message teacher")
- Dark mode preferred (easier on eyes for evening access)

### Unavailability Workflow

- Step 1: "Mark yourself unavailable" - date range + reason
- Step 2: "Affected lessons:" - itemized list (lesson time, student, status)
- Step 3: Confirm - "Notifications sent, credits posted"
- Success state: green checkmark, summary of actions taken
...
- [ ] Color: Does this respect the primary teal + semantic color scheme?
- [ ] Spacing: Used CSS variables (`--space-*`), not magic numbers?
- [ ] Motion: Does this animate with purpose (not scattered)?
...
## Part 13: CSS Variables Template
