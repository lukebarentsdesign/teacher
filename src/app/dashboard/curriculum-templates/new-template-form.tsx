...
import { createCurriculumTemplateAction } from "./actions";

type LessonType = { id: string; name: string };

export function NewTemplateForm({ lessonTypes }: { lessonTypes: LessonType[] }) {
  const [error, action, pending] = useActionState(createCurriculumTemplateAction, undefined);
    ...
    <form action={action} className="space-y-3 rounded-xl bg-white p-6 shadow-sm">
      <div>
        ...
        <input
          id="title"
          name="title"
          placeholder="e.g. Flute Grade 1 — standard"
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1">
          <label htmlFor="lessonTypeId" className="block text-sm font-medium text-neutral-700">
            Suggest for lesson type (optional)
          </label>
          <select
            id="lessonTypeId"
            name="lessonTypeId"
