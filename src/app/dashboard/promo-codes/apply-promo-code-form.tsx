...
export function ApplyPromoCodeForm({ subscriptions }: { subscriptions: Subscription[] }) {
  const [error, action, pending] = useActionState(applyPromoCodeAction, undefined);
  ...
  return (
    <form action={action} className="flex flex-wrap items-end gap-2 rounded-xl bg-white p-4 shadow-sm">
      ...
      <div className="w-32">
        <label htmlFor="baseAmount" className="block text-xs font-medium text-neutral-700">
          Bill amount (£)
        </label>
        <input
          id="baseAmount"
