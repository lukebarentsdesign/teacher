...
- **The optional first-student step reuses `createStudentWithRelationshipsAction` directly** (the same action the full student wizard uses) with a trimmed payload (`GUARDIAN` payment responsibility only, `discipline: "General"`) rather than re-implementing student creation — deliberately doesn't expose the SELF/SCHOOL payment-responsibility branches or under-18 emergency-contact logic that the full wizard handles; a teacher who needs those uses the full wizard later from `/dashboard/students/new`, unaffected by this trimmed version.
- **Dynamic nav: "short core + More overflow," not the spec's literal short-list-only** — user-confirmed decision. The spec's Section 5 table only lists ~9 items total across both archetypes, but this codebase has ~35 shipped features (most from the two roadmap-v2 passes) that the spec's own Phase 3 disclosure table doesn't account for. Hiding those entirely behind trigger conditions the spec never defined would have made real, working, tested features unreachable via navigation. Instead: a short archetype-based core list (`CORE_SOLO`/`CORE_GROUP` in `src/app/dashboard/sidebar.tsx`) plus any earned Phase 3 items render in the primary "Menu" group; everything else lives in a collapsible "More" section, closed by default — nothing built is ever unreachable, but a fresh teacher's first impression is still the short list the spec asks for.
- **Pre-existing accounts (`archetype: null`) fall back to the full flat list**, not a guessed-at short nav — an account that predates this onboarding flow (or is mid-onboarding) has no archetype to branch on, and guessing one would risk hiding features an existing user already relies on. This is the one case where `SidebarContent` renders the old undifferentiated "Everything" list.
- **Phase 3 trigger checks live in `src/app/dashboard/layout.tsx`**, computed server-side once per request and passed down as a plain `string[]` of earned keys — kept as simple DB counts/existence checks (3+ lessons with one student, a linked `SCHOOL` location, a `WAITLISTED` booking having ever existed, `organisationId` set, tax-season date window) rather than new tracking infrastructure, per the spec's own framing. `isWithinTaxSeasonWindow` (March–April) is the one pulled out as a pure, unit-tested function since it's the only trigger with real logic beyond "does a row exist."
- **Phase 2 dashboard cards** (`src/app/dashboard/dashboard-cards.tsx`) use a JSON `dismissedCards` map (cardId → dismissal timestamp) rather than a plain dismissed-list, specifically so a card can resurface after a cooldown if its condition is still true (`shouldShowCard`, pure/unit-tested) — "don't nag about Stripe daily, but do come back weekly until it's connected" needed a timestamp, not just a boolean.
- **Not built from the spec**: the `OutOfScopeSignup` review UI (spec only says the data is "worth reviewing periodically," doesn't ask for a dedicated screen — querying the table directly is enough for now); most of the Phase 3 table's other triggers (course library, venue fees, assessments) — only the ones with an existing dedicated nav item to promote were wired up, since the others don't have a single obvious "earned" surface to attach to yet.

## Loose Ends: Admin Review, Self-Service Booking, Course Payments

- **`/dashboard/out-of-scope-signups` is gated by a plain `ADMIN_EMAILS` env var** ([src/lib/admin.ts](src/lib/admin.ts)), not a real role system — this app has no admin/superuser concept anywhere else, and the spec frames this screen as the *product owner* periodically reviewing signal across all teachers, not something any individual teacher needs to see about themselves. Building a whole role/permission tier for one internal screen would be disproportionate; a comma-separated allowlist is enough. Not linked from the sidebar (would 404 for everyone else).
- **Self-service group-class booking on the microsite (`/parent/students/[id]/classes`) is scoped to classes the student is already a `GroupClassMember` of** — deliberately not a browse-and-join-any-class catalog (that's discovery/capacity-before-membership, a bigger feature). Reuses the exact same `resolveBookingStatus`/`pickPromotionCandidate` pure functions as the teacher-facing booking panel (`src/lib/group-booking.ts`), just called from a guardian-authenticated action instead of a teacher-authenticated one.
- **Course purchases now have a real Stripe Checkout path** (`src/lib/course-payments.ts`), alongside the existing manual `recordCoursePurchaseAction` — same destination-charge pattern as `createParentPaymentCheckoutSession`. The shared webhook (`src/app/api/webhooks/stripe/route.ts`) distinguishes the two "payment"-mode Checkout flows by a `courseId` metadata key rather than a new webhook endpoint. **Still deliberately doesn't touch `LedgerEntry`** on the Stripe path either — same reasoning as the manual path in "Sellable Course Content" above (a course purchase may have no `Subscription` to attach a ledger line to); the webhook creates `CoursePurchase` directly, idempotent on the same `(courseId, payerId)` unique constraint the manual path already relies on.
